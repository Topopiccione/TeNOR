...
class IR_helper
	...
	def queryIR(ir_address, simulation, randNum, debugprint, overcommiting)
		...
		# these are the arrays/hashes that are going to be written to disk
		pop_id_array_g = Array.new
		pop_detail_array_g = Array.new
		pop_link_id_array_g = Array.new
		pop_link_detail_array_g = Array.new
		hypervisors_detail_hash_g = Hash.new

		ir_address.each do |base_ir_address|

			# these are local and are going to be merged to the *_g ones
			...
			pop_id_array_g += pop_id_array
			pop_detail_array_g += pop_detail_array
			pop_link_id_array_g += pop_link_id_array
			pop_link_detail_array_g += pop_link_detail_array
			hypervisors_detail_hash_g = hypervisors_detail_hash_g.merge( hypervisors_detail_hash )
		...
		outputHash = Hash.new
		outputHash.store( "PoP_id", pop_id_array_g )
		outputHash.store( "PoP_detail", pop_detail_array_g )
		outputHash.store( "PoP_link_id", pop_link_id_array_g )
		outputHash.store( "PoP_link_detail", pop_link_detail_array_g )
		outputHash.store( "PoP_aggregate_resources", hypervisors_detail_hash_g )
